...
async def test_process(
    vaa_delivery: IVaaDelivery, relays_repo: ITransactionsRepo, test_db: Database
) -> None:

    test_parsed_vaa: ParsedVaa = vaa_delivery._parse_vaa(vaa=constant.TEST_VAA_BYTES)

    await vaa_delivery.process(vaa=constant.TEST_VAA_BYTES)
...
#     vaa_delivery: IVaaDelivery  # Need a different vaa_delivery... -> vaa_delivery_with_mock
# ) -> None:
...
@pytest.mark.asyncio
async def test_parse_vaa(vaa_delivery: IVaaDelivery) -> None:

    test_parsed_vaa = vaa_delivery._parse_vaa(vaa=constant.TEST_VAA_BYTES)

    assert isinstance(test_parsed_vaa, ParsedVaa)


@pytest.mark.asyncio
async def test_parse_payload(vaa_delivery: IVaaDelivery) -> None:

    test_parsed_vaa = vaa_delivery._parse_payload(payload=constant.TEST_VAA_PAYLOAD)
