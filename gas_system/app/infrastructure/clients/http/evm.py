...
class EvmClient(IEvmClient):
    ...
    async def update(self, data: bytes, dest_chain_id: int) -> TransactionHash:
        ...
        try:
            signed_transaction = await self.__craft_transaction(
                data=data, contract=contract, web3_client=web3_client
            )
    ...
    async def __craft_transaction(
        self, data: bytes, contract: Contract, web3_client: Web3
    ) -> SignedTransaction:
        ...
        # function setSendFees(uint16[] memory _destChainIds, uint256[] memory _fees)

        x = y = "hi"

        transaction = contract.functions.setSendFees(x, y).buildTransaction(
            {
